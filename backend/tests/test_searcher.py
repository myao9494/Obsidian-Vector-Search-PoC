...
from app.searcher import VectorSearcher, SearchMode, SearchResponse
...
def test_document_search(indexed_vault):
    ...
    assert top_item.score >= -1.0 and top_item.score <= 1.0001
...
def test_chunk_search_with_context(indexed_vault):
    ...
    assert top_chunk.chunk_index is not None
    assert top_chunk.hit_text != ""
    # contextオブジェクトが存在すること
    assert top_chunk.context is not None
    assert "current" in top_chunk.context
